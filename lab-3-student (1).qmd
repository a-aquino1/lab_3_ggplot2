...
ggplot(data = surveys,
  ...
  facet_wrap(~species) +
  labs(title = "Weight and Hindfoot Length Correlation by Species",
       subtitle = "Hindfoot Length",
       x = "Weight",
       y = " ")
## created a scatter plot using the given layout in the previous code. Added on to the code by adding labels, a title, and creating multiple plots by species.
```
...
ggplot(data = surveys,
  ...
  labs(x = "Weight of the Rodents",
       y = "Rodent Species")
## created a box plot and updated the aesthetics and rotated the text 45 degrees as well as added the jitter to make the plot more readable.
```
...
ggplot(data = surveys,
  ...
  labs(x = "Rodent Species",
       y = "Weight of the Rodents")
## updated the previous code to be flipped
```
